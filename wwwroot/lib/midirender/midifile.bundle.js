    ...
    function MIDIFile(buffer, strictMode) {
        ...
        if (!buffer) {
            ...
            if (buffer.byteLength < 25) {
                throw new Error(
                    'A buffer of a valid MIDI file must have at least 25 bytes.'
                );
            }
            ...
            for (i = 0, j = this.header.getTracksCount(); i < j; i++) {
                if (strictMode && curIndex >= buffer.byteLength - 1) {
                    throw new Error(
                        "Couldn't find data corresponding to track #" + i + '.'
                    );
                }
            ...
            if (strictMode && curIndex !== buffer.byteLength) {
                throw new Error('It seems that the buffer contains too much data.');
            }
        }
    }

    // Просто реєструємо глобально для браузера
    if (typeof window !== 'undefined') {
        window.MIDIFile = MIDIFile;
    }


// Універсальний експорт (працює і в браузері, і в Node.js)
(function (global) {
